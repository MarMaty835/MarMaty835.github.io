...
async function helpsResponse() {
    const val = getValues(inputString.value);
    for (let i = 0; i < val.length; i++)
        helpVars.innerHTML += `<li class="suggestion-item">
            <span class="suggestion-text" id="help${i}">${val[i].name}</span>
            </li>`;
    for (let i = 0; i < val.length; i++)
    {
        const el = document.getElementById(`help${i}`);
        el.onmousedown = () => pick(val[i].name);
    }
}
...
function pick(name)
{
    ...
    tit.textContent = `${source.properties.name}`
    des.textContent = `${source.properties.descr}`;
    lat.textContent = `${source.properties.loc[1]}`;
    lng.textContent = `${source.properties.loc[0]}`;
    pop.textContent = `${source.properties.population}`;
    dat.textContent = `${source.properties.founded}`;
...
async function load() {
    ...
    {
        if (geojson.features[i].properties.population < 100000)
            continue;
        const res = await fetch(`https://ru.wikipedia.org/api/rest_v1/page/summary/${geojson.features[i].properties.name}`);
        let json = await res.json();
        if (!(json.description.includes("город") || json.description.includes("Город")))
        {
            const res1 = await fetch(`https://ru.wikipedia.org/api/rest_v1/page/summary/Город ${geojson.features[i].properties.name}`);
            if (res1.ok)
                json = await res1.json();
        }
        geojson.features[i].properties.descr = json.extract;
        ...
        map.on('mousedown', `${geojson.features[i].properties.name}`, (ev) => {
            ...
            tit.textContent = `${geojson.features[i].properties.name}`
            des.textContent = `${geojson.features[i].properties.descr}`;
            lat.textContent = `${geojson.features[i].properties.loc[1]}`;
            lng.textContent = `${geojson.features[i].properties.loc[0]}`;
            pop.textContent = `${geojson.features[i].properties.population}`;
            dat.textContent = `${geojson.features[i].properties.founded}`;
